...
// Moment of inertia (kg·m²) — determines rotational resistance (arcade-style: moderate for stability)
const Ixx = 4500;   // roll  (higher for more stable roll)
const Iyy = 18000;  // pitch (higher to prevent wild pitch)
const Izz = 20000;  // yaw   (higher for stable yaw)
...
// Max moments generated by control surfaces at reference q (N·m per unit deflection) - arcade-style: balanced power
const M_elevator = 180000;   // pitch moment from elevator (strong but controlled)
const M_rudder = 75000;      // yaw moment from rudder (moderate)
const M_aileron = 140000;    // roll moment from ailerons (strong but controlled)

// Control surface slew rate (max deflection change per second, -1..1 range) - arcade-style: fast but smooth
const CONTROL_SLEW_RATE = 5.5;   // units/sec (fast but not instant)
