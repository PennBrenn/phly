...
const MAX_THRUST_SL = 75000;   // N   static sea-level thrust (twin engines - increased)
const WING_AREA = 17.3;        // m²
const WING_SPAN = 8.1;         // m
const ASPECT_RATIO = WING_SPAN * WING_SPAN / WING_AREA;
const GRAVITY = 9.81;          // m/s²

// Moment of inertia (kg·m²) — determines rotational resistance (arcade-style: higher for less maneuverability)
const Ixx = 6000;   // roll  (higher for more stable roll)
const Iyy = 22000;  // pitch (higher to prevent wild pitch)
const Izz = 25000;  // yaw   (higher for stable yaw)
...
// Pitch (arcade-style: stronger damping to prevent oscillation)
const Cm_alpha = -0.5;   // static longitudinal stability (moderate - prevents wild pitch)
const Cm_q = -8.0;       // pitch damping (much higher - stops pitch oscillation)
const Cm_alphadot = -2.5; // AoA rate damping (higher to smooth pitch)

// Yaw (arcade-style: moderate damping)
const Cn_beta = 0.08;    // directional stability (moderate weathervane effect)
const Cn_r = -0.25;      // yaw damping (higher to prevent spinning)
const Cn_da = -0.005;    // adverse yaw from aileron (very minimal)

// Roll (arcade-style: strong damping to prevent barrel rolls)
const Cl_beta = -0.05;   // dihedral effect (moderate)
const Cl_p = -0.45;      // roll damping (much higher - prevents uncontrolled rolls)
const Cl_r = 0.01;       // roll from yaw rate (very minimal coupling)

// ═══════════════════════════════════════════════════════════════════════════════
// ─── Control Surface Parameters ──────────────────────────────────────────────
// ═══════════════════════════════════════════════════════════════════════════════

// Max moments generated by control surfaces at reference q (N·m per unit deflection) - arcade-style: reduced maneuverability
const M_elevator = 140000;   // pitch moment from elevator (reduced)
const M_rudder = 60000;      // yaw moment from rudder (reduced)
const M_aileron = 110000;    // roll moment from ailerons (reduced)

// Control surface slew rate (max deflection change per second, -1..1 range) - arcade-style: slower for less maneuverability
const CONTROL_SLEW_RATE = 4.0;   // units/sec (slower response)
...
// Angular momentum damping from aerodynamic surfaces (baseline) - arcade-style: strong damping
const AERO_DAMPING_SCALE = 0.75;

// Sideslip damping / weathervane (force that aligns velocity with nose)
const SIDESLIP_FORCE_SCALE = 2.5;

// Cross-axis inertia coupling scale (reduced to prevent barrel rolls from pitch)
const INERTIA_COUPLING_SCALE = 0.08;
