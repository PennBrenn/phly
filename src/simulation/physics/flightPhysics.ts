...
// Moment of inertia (kg·m²) — determines rotational resistance (reduced for easier control)
const Ixx = 3500;   // roll  (lighter for easier roll)
const Iyy = 18000;  // pitch (reduced for more responsive pitch)
const Izz = 20000;  // yaw   (reduced for easier yaw)
...
// Pitch (reduced damping for easier control)
const Cm_alpha = -0.5;   // static longitudinal stability (less restoring force)
const Cm_q = -6.0;       // pitch damping (reduced - less resistance)
const Cm_alphadot = -2.0; // AoA rate damping (reduced)

// Yaw (reduced for easier control)
const Cn_beta = 0.08;    // directional stability (reduced weathervane)
const Cn_r = -0.12;      // yaw damping (reduced resistance)
const Cn_da = -0.02;     // adverse yaw from aileron (reduced)

// Roll (reduced damping for snappier response)
const Cl_beta = -0.05;   // dihedral effect (reduced)
const Cl_p = -0.20;      // roll damping (reduced for faster roll)
const Cl_r = 0.04;       // roll from yaw rate (reduced coupling)

// ═══════════════════════════════════════════════════════════════════════════════
// ─── Control Surface Parameters ──────────────────────────────────────────────
// ═══════════════════════════════════════════════════════════════════════════════

// Max moments generated by control surfaces at reference q (N·m per unit deflection) - increased for better authority
const M_elevator = 180000;   // pitch moment from elevator (50% increase)
const M_rudder = 70000;      // yaw moment from rudder (55% increase)
const M_aileron = 120000;    // roll moment from ailerons (50% increase)

// Control surface slew rate (max deflection change per second, -1..1 range) - faster for quicker response
const CONTROL_SLEW_RATE = 5.0;   // units/sec (increased for snappier controls)
...
// Angular momentum damping from aerodynamic surfaces (baseline) - reduced for easier control
const AERO_DAMPING_SCALE = 0.6;
